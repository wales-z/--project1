...
#####################################################################################################
    m, n, c = image.shape
    m_padding = (filter.shape[0] - 1) // 2
    n_padding = (filter.shape[1] - 1) // 2
    padded_image = np.pad(image, ((m_padding, m_padding),
                                  (n_padding, n_padding), (0, 0)), 'constant')

    filtered_image_height = padded_image.shape[0] - filter.shape[0] + 1
    filtered_image_width = padded_image.shape[1] - filter.shape[1] + 1
    filtered_image = np.zeros(
        [filtered_image_height, filtered_image_width, image.shape[2]])

    for channel in range(c):
        for height in range(m):
            for width in range(n):
                filtered_image[height, width, channel] = np.sum(
                    np.multiply(padded_image[height:height + filter.shape[0], width:width + filter.shape[1], channel], filter))
